...
{# 
    Run using
    dbt run-operation get_last_manifest --args '{"artifacts_location": "balboa.dbt_artifacts.artifacts"}' | sed -e '1,1d' > logs/manifest.json
#}
{# Once this is completed, deferral and state modifiers are available using --state logs #}

{% macro get_last_manifest(artifacts_location) %}
